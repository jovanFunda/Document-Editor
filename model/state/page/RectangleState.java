	...
	public void mousePressed(MouseEvent e) {
		...
		if (e.getButton()==MouseEvent.BUTTON1){
			 if (page.getPageModel().getDeviceAtPosition(position)==-1){
				...
				abFactory.chooseSlot(e, MainFrame.getInstance().getLastSelectedPageView().getPage(), cntRec);
			
				MainFrame.getInstance().getTree().setSelectedNode(MainFrame.getInstance().getLastSelectedPageView().getPage().getPageTree());	

				 
				 RectanglePainter painter = new RectanglePainter(position);
				 
				 painter.getDevice().setName("Rectangle " + cntRec);
